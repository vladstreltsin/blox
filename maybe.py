""" Taken and adapted from https://github.com/ekampf/pymaybe
...
# Python 2 compatibility functions
def cmp(a, b):
    return (a > b) - (a < b)
...
class Nothing(Maybe):
    ...
    def get(self, exc: T.Optional[Exception] = None):
        if exc is not None:
            raise exc
        raise NothingError('Calling get() on None')

    def or_else(self, els=None):
        if callable(els) and not isinstance(els, type):
            return els()
