...
class _Label:
    ...
    def __init__(self, text: str, height: tp.Optional[int] = None, width: tp.Optional[int] = None):
        self.text = text
...
class _LabelsSection:
    ...
    def __init__(self, owner: tp.Union[Port, Block]):
        self.__labels = dict()
...
class _NodeSize:
    ...
    def minimal_width(self):
        return 6.*len(self.block.name) + 20.
    ...
    @minimum.setter
    def minimum(self, value: tp.Tuple[float, float]):
        value = tuple(value)
        assert len(value) == 2
        x = float(value[0])
        y = float(value[1])
...
class _BlockVis:
    ...
    def __init__(self, _block: Block, _diagram: ElkDiagram):
        self.__block = _block
        self.__diagram = _diagram
    ...
    def to_dict(self):
        ...
        return {"id": str(id(self.block)),
                "children": [self.diagram[child].to_dict() for child in self.block.blocks] if self.show else [],
                "labels": [label.to_dict() for label in self.labels],
                "ports": [self.diagram[port].to_dict() for port in chain(self.block.In, self.block.Out)],
                "edges": [{"id": f"{id(p1)}_{id(p2)}", "sources": [f"{id(p1)}"], "targets": [f"{id(p2)}"]}
                          for p1, p2 in self.block.links()] if self.show else [],
                "layoutOptions": {
...
class ElkDiagram:
    ...
    def __init__(self, block: Block):
        ...
        def _rec_export(blk):
            ...
            for port in chain(blk.In, blk.Out):
    ...
    def __getitem__(self, item: tp.Union[Block, Port]):
