...
from blox_old.core.block.base import Function, AtomicFunction
...
#          bnn.Conv2d(in_channels, out_channels,
...
class Hsigmoid(AtomicFunction):
    def __init__(self, inplace=True, **kwargs):
        super(Hsigmoid, self).__init__(**kwargs, In=1, Out=1)
...
class eSEModule(Function):

    def __init__(self, channel, reduction=4, **kwargs):
        super(eSEModule, self).__init__(In=1, Out=1, **kwargs)
        self.avg_pool = bnn.AdaptiveAvgPool2d(1)
        self.fc = bnn.Conv2d(channel, channel, kernel_size=1, padding=0)
